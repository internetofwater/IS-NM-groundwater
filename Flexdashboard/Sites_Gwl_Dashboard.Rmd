...
pacman::p_load(tidyverse, flexdashboard,maps,leaflet,sf,plotly,shiny,viridis,shinyWidgets,scales, data.table, DT)
#$library(tidyverse)
...
sites.summary.static <- fread("../Processed/sites.summary.static.csv")
...
sliderInput("Depth", "Well Depth (ft)",
            min = -1, max = 7400, value = c(0,1000), step = 10)
...
pickerInput(
   inputId = "Site",
   label = "To search for specific site number, deselect all counties, then type below", 
    choices = sitelist,
   selected=NULL,
   options = list(
      `live-search` = TRUE,
      `size` = 10, 
      `title` = "Type Site Number")
)
```
...
- Site level and groundwater data go back as far as May 1st, 1900. In order to see sites with information in the 20th century, type a date in the box or search for a specific year or day.

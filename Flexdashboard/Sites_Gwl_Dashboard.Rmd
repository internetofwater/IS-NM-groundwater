...
click_marker <- eventReactive(input$map_marker_click, {input$map_marker_click

  return(x$SiteNo)

})

n <- eventReactive(input$goButton, {input$SiteNo}, ignoreNULL = FALSE)
n

output$well <- renderPlot({
  ggplot(data_for_plot(), aes(x=Date, y = Depth.to.Water.Below.Land.Surface.in.ft.)) +
    geom_point()
